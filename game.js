...
const gunModels = WEAPONS.map((w, i) => {
  const g = new THREE.Group();
  const bodyColor = [0x4a90d8, 0xe8873d, 0xbfa5ee, 0xff7ad9][i];
...
document.addEventListener("keydown", e => {
  ...
  const num = { Digit1: 0, Digit2: 1, Digit3: 2, Digit4: 3 }[e.code];
...
function updatePlayer(dt) {
  ...
  /* holding the button down keeps the Blaster firing */
  if (mouseDown && WEAPONS[player.weapon].auto) playerShoot();
